...
class IperfClient:
    ...
    def run(self):
        ...
        while True:
            ...
            if initStr:
                print(initStr)
            if 'iperf Done.' in initStr:  # 初始化成功
                break
        # cancelTimeout()
    ...
    def __del__(self):
        self.stop()
        atexit.unregister(self.stop)  # 移除退出处理
        print('IperfClient 析构！')


if __name__ == "__main__":
    print("start")
    # server = IperfServer(r"iperf3.exe")
    # server.run()  # 会发生阻塞，若在GUI中需在线程中启动，通过线程控制服务启动和停止
    # server.stop()

    client = IperfClient(r"iperf3.exe", argsStr="192.168.2.147")
    client.run()  # 需增加结果解析
    client.stop()
    print('end')
