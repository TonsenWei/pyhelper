...
from PyQt5.QtWidgets import QApplication, QWidget, QLCDNumber, QFrame, QGridLayout

from examples.gui.meter_gui.meter_gui import Ui_meterGui
...
class MainWidget(QWidget):
    def __init__(self, parent=None):
        ...
        self.meterGui = Ui_meterGui()
        self.meterGui.setupUi(self)
        meterGuigridLy = QGridLayout(self.meterGui.widget)
        self.meterWidget = Dashboard()
        meterGuigridLy.addWidget(self.meterWidget)
        self.initSignal()
    ...
    def setValue(self):
        valueStr = self.meterGui.lineEditSetValue.text()
        try:
            value = float(valueStr)
            print("setvalue")
            self.meterWidget.setValue(value)
            self.meterWidget.update()
