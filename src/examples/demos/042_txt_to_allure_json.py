                ...
                if stripedLine != "":
                    ...
                    if reportType == ONE_CASE or reportType == ONE_SUIT:
                        ...
                        if currentType == TEST_INOF:
                            ...
                            stopInt = int((startTimeF + caseCostTime) * 1000)
                            perCaseCostTime = TimeUtil.time_simple(float(caseCostTime))
                            totalCostTime += caseCostTime
                        elif currentType == ATTACHMENT:
                            ...
                            attachments.clear()
                    line = reportTxt.readline()
            # html_report.modify_suit_time(TimeUtil.time_simple(totalCostTime - taskBeforeCostTime))
            # html_report.modify_header(time_str=TimeUtil.time_simple(totalCostTime),
            #                           total=str(total_cases) + " total, ",
            #                           error=str(fail_cases) + " error, ",
            #                           ignored="0 ignored, ",
            #                           passed=str(pass_counter) + " passed, ",
            #                           rate="pass rate: " + str(pass_rate) + "%")
            # html_report.output(htmlDir)  # 写到html文件


if __name__ == "__main__":
    TxtToAllureJson.genAllureData(r"D:\projects\python\pyside2_prjs\std_autotest\output\report\2023-03-01_14-11-04\report.txt", "./allure_data/")
