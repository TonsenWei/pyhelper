...
def evalTest():
    ret = eval(
        r"['D:\\projects\\python\\pyside2_prjs\\std_autotest\\output\\report\\2023-02-10_11-43-25\\images\\2023-02-10_11-43-29_469.png', 'D:\\projects\\python\\pyside2_prjs\\std_autotest\\output\\report\\2023-02-10_11-43-25\\images\\2023-02-10_11-43-29_701_2023-02-10_11-32-51_976.png']")
    print(ret[0])
    print(ret[1])
...
def lineDistance(startPos, endPos):
    xDis = startPos[0] - endPos[0]  # x距离
    yDis = startPos[1] - endPos[1]  # Y距离
    dis = math.sqrt(pow(xDis, 2) + pow(yDis, 2))
    print(f"dis={dis}")
    return dis


def nearLineDistance(startPos, endPos, nearLen):
    """
    获取旁边线段的距离
    :param startPos:
    :param endPos:
    :param nearLen: 与原线的距离
    :return:
    """
    xDis = startPos[0] - endPos[0]  # X距离
    yDis = startPos[1] - endPos[1]  # Y距离
    dis = math.sqrt(pow(xDis, 2) + pow(yDis, 2))
    print(f"dis={dis}")
    nearLineDis = math.sqrt(pow(dis, 2) + pow(nearLen, 2))
    print(f"nearLineDis = {nearLineDis}")
    return nearLineDis
